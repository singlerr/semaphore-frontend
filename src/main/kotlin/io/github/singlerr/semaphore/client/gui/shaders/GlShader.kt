package io.github.singlerr.semaphore.client.gui.shaders
...
internal class GlShader(
    ...
    internal fun doBindTexture(textureUnit: Int, textureId: Int) {
        ...
        prevTextureBindings.computeIfAbsent(textureUnit) {
            GL11.glGetInteger(GL_TEXTURE_BINDING_2D)
        }
...
internal class DirectFloatMatrixUniform(location: Int) :
    DirectShaderUniform(location), FloatMatrixUniform {
...
internal class DirectSamplerUniform(
    ...
    init {
        shader.withProgramBound { DirectIntUniform(location).setValue(textureUnit) }
    }
